...
class ${className}Controller {
    ...
    def index(Integer max) {
		if (!session.getAttribute("controller_active").equals("${className}")){
			session.setAttribute("controller_active", "${className}");
			session.setAttribute("${propertyName}_search_rows", 10);
			session.setAttribute("${propertyName}_search_word", "");
			session.setAttribute("${propertyName}_search_offset", 0);
		}
    ...
    def save(${className} ${propertyName}) {
		...
		/*
		int id = 0;
        if (${className}.createCriteria().get {projections {max "id"}} as Integer != null){
           id = ${className}.createCriteria().get {projections {max "id"}} as int
        }
        id++
         ${propertyName}.id = id
        */
